...
uint32_t counter = 0; //counts the number of interupts,will be incremented with each intterupt,sits on a heap
//uint32_t means unsigned 32 bits for count(variable wich takes 32 bits)
void initTimer0() 
{ 
...
ISR( TIMER0_COMPA_vect ) //interupts should be short, never put delay here
{ 
    counter++; 
...
{ 
    counter++; 
    PORTB |= _BV( PB2 ) | _BV( PB3 ) | _BV( PB4 ) | _BV( PB5 ); //will start switching leds on and off
} 
