...
void displayRandomNumbers(int slotCount) {
    int numbers[4]; //arrray for storing rand numbers 
    for (int i = 0; i < slotCount; i++) {
        numbers[i] = rand() % 10; // Generate random number between 0 and 9 for each slot
    }
 
    // Display all numbers simultaneously on segment 
    for (int i = 0; i < slotCount; i++) {
        writeNumberToSegment(i, numbers[i]);//write each random number on each segment of the display
    }
 
    // Check if all numbers are the same
    int win = 1; //flag if all numbers the same
    for (int i = 1; i < slotCount; i++) { 
        if (numbers[i] != numbers[0]) { //compare each number with 1st num
            win = 0; //if any number is different,set flag to 0.
            break; //exit the loop,as we found a mismatch.
        }
    }
    if (win) { //if win still 1 ,all numbers the are same 
        //determine the amount of win,based on the number of slots
        int winAmount = (slotCount == 2) ? WIN_AMOUNT_2 : (slotCount == 3) ? WIN_AMOUNT_3 : WIN_AMOUNT_4;//if slotCount ==2 true, assign win_amount_2 to winAmount,if false assign win_amount_3 and if it's false assign win_amount_4.
        coins += winAmount; //add the win amount to player's coins
...
void checkGameOver() {
    if (coins <= 0) { //if player's amount of coind less than 0, print loss statement 
    ...
    } else if (coins >= MAX_COINS) { //if player's coins reached max coins number, print win statement.
...
void checkWin(int numbers[], int slotCount) {
    ...
    for (int i = 1; i < slotCount; i++) { //loop through remaining numbers 
        if (numbers[i] != targetNumber) { //if any number is diff from the target number
    ...
    while (1) {       
        if (bit_is_clear(PINC, BUTTON1)) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                _delay_ms(800); // it holds the number on display 800ms
